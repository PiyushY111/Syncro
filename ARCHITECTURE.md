...
**Whiteboard sync is last-write-wins, not a CRDT/OT merge.** `whiteboard:update` (`server/src/socket/whiteboardHandler.js`) broadcasts a client's full `pages`/`currentPageId` payload to every other socket in the room; each recipient replaces its local state with whatever arrives last. There's no operational-transform or conflict-free-replicated-data-type layer reconciling concurrent edits, so two people drawing on the same page at the same time can overwrite each other's strokes — the version that reaches the server (and other clients) last simply wins, with no merge and no conflict warning. The REST save path (`saveWhiteboard`, `server/src/controllers/whiteboard/whiteboardSave.js`) has an optimistic-lock `expectedVersion` check for its own persistence, but that only protects the persisted document against a stale full save; the live socket broadcast has no equivalent guard.
...
**Active MITM at the TLS trust boundary is also out of scope.** An attacker who can already intercept the TLS session itself (a compromised CA, or a device with an attacker-installed root certificate) can intercept the ECDH handshake the same way. Closing that would require client certificate pinning, which isn't implemented here and is arguably not worth the operational cost for this application. More precisely: the ECDH handshake itself is unauthenticated — the server's ephemeral public key (`handleHandshake` in `server/src/services/shieldEngine.js`) is sent as-is, with no signature and no pinning against a known key, so a party already able to intercept and rewrite the handshake (i.e. already at the same trust boundary as an active TLS MITM) can substitute its own key and transparently proxy the "encrypted" channel. Shield's protection is therefore strictly bounded by whatever TLS already provides against an active attacker; it adds nothing beyond TLS on that specific axis, only against the passive-observer scenarios described above.

**Operational limitation:** the anti-replay nonce store and session store fall back to an in-process `Map`/`Set` if Redis is unreachable (§7.5). That's correct for a single instance, but doesn't provide replay protection across horizontally scaled instances without Redis configured. Separately, whichever store holds it: session keys (`encKeyHex`/`authKeyHex`) are stored as plain hex strings, not further encrypted at rest — anyone with read access to that Redis instance (or the in-memory fallback's process memory) can recover every active Shield session's keys.
