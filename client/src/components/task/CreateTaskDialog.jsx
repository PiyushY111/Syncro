...
import TaskRecurrenceSelector from '@/components/task/TaskRecurrenceSelector';
import TaskTypePrioritySelector from '@/components/task/TaskTypePrioritySelector';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 dark:bg-black/60 backdrop-blur">
            <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg shadow-lg w-full max-w-md p-6 text-zinc-900 dark:text-white text-left">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <TaskTypePrioritySelector
                        type={formData.type}
                        priority={formData.priority}
                        onChangeType={(type) => setFormData({ ...formData, type })}
                        onChangePriority={(priority) => setFormData({ ...formData, priority })}
                    />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <select value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })} className="w-full rounded dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-zinc-900 dark:text-zinc-200 text-sm mt-1 focus:outline-none cursor-pointer" >
                                {stages.map((stageId) => (
                    ...
                    <TaskRecurrenceSelector
                        ...
                        onChangeRecurring={(isRecurring) => setFormData({ ...formData, isRecurring })}
                        onChangeRecurrence={(recurrence) => setFormData({ ...formData, recurrence })}
                    />
