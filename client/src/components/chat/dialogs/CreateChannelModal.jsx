...
import { X, Hash, Lock, Image } from 'lucide-react';
...
export default function CreateChannelModal({ isOpen, onClose, workspaceId, onChannelCreated }) {
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl text-left animate-stiff-pop">
                ...
                <form onSubmit={handleSubmit} className="space-y-3 text-xs">
                    <div>
                        ...
                        <input
                            ...
                            required
                            className="w-full bg-zinc-50 dark:bg-zinc-800 border rounded-lg p-2 text-zinc-900 dark:text-white"
                        />
                    </div>

                    <div>
                        <label className="block font-semibold text-zinc-700 dark:text-zinc-300 mb-1">Channel Icon Image URL (Optional)</label>
                        <input
                            type="url"
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows={2}
                            className="w-full bg-zinc-50 dark:bg-zinc-800 border rounded-lg p-2 text-zinc-900 dark:text-white resize-none"
                        />
                    </div>

                    <div className="flex items-center justify-between p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-800/60 border">
                        <div>
                        ...
                        <input type="checkbox" checked={isPrivate} onChange={(e) => setIsPrivate(e.target.checked)} className="size-4 text-indigo-600 rounded cursor-pointer" />
                    </div>
                    ...
                    <div className="flex justify-end gap-2 pt-1">
                        ...
                        <button type="submit" disabled={loading || !name.trim()} className="px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-medium disabled:opacity-50">
